...
agent = Agent(
    ...
    seed="Auth API Specialist 23422424",
    port=8002,
...
# 2. DEFINE THE PROMPT
SYSTEM_PROMPT = f"""
You are an API security specialist, specialization in auth.
"""
...
async def handle_batch(ctx: Context, logs: SpecialistRequest) -> Dict[str, List[Dict]]:
    ...
    """
    ctx.logger.info(f"[AUTH] Received batch of {len(logs.logs)}")
    ...
    try:
        ...
        ctx.logger.info(f"[AUTH] Mitigations: {len(mitigations)} in {latency} seconds")
        ...
        ctx.logger.error(
            f"[AUTH] Error parsing JSON from Groq: {e}. Response was: {llm_output_str}")
        return []
