...
agent = Agent(
    ...
    seed="Search API Specialist 1223923",
    port=8003,
...
# 2. DEFINE THE PROMPT
SYSTEM_PROMPT = f"""
You are an API security specialist, specialization in search.
"""
...
async def handle_batch(ctx: Context, logs: SpecialistRequest) -> Dict[str, List[Dict]]:
    ...
    """
    ctx.logger.info(f"[SEARCH] Received batch of {len(logs.logs)}")
    ...
    try:
        ...
        ctx.logger.info(f"[SEARCH] Mitigations: {len(mitigations)} in {latency} seconds")
        ...
        ctx.logger.error(
            f"[SEARCH] Error parsing JSON from Groq: {e}. Response was: {llm_output_str}")
        return []
